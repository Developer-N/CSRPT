...
@Database(entities = [DownloadedPrayTimesEntity::class], version = 2, exportSchema = false)
abstract class DownloadedPrayTimesDB : RoomDatabase() {
    ...
    companion object {
        ...
        fun getInstance(context: Context): DownloadedPrayTimesDB {
            if (instance == null) {
                synchronized(DownloadedPrayTimesDB::class) {
...
data class DownloadedPrayTimesEntity(
    ...
    fun toDoubleAndFix(time: String, fixMin: Int): Double =
        toDouble(fixTime(time, fixMin))
}
...
interface DownloadedPrayTimesDAO {
    ...
    suspend fun getCities(): MutableList<Int>?
