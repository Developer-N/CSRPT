...
class DownloadFragment : Fragment() {
    ...
    fun updateRecycler() {
        ...
        if (sura > 0)
            (binding.recyclerQuranDownload.layoutManager as LinearLayoutManager)
                .scrollToPositionWithOffset(sura - 1, 0)
    }
            ...
            private fun download(chapter: ChapterEntity, downloadID: Int, position: Int) {
                ...
                val downloadFile =
                    getSelectedQuranDirectoryPath(requireContext()) + "/" +
                            folders[spinnerSelectedPosition] + "/" + getSuraFileName(chapter.sura)
                val folderPath =
