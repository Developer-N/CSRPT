...
import com.byagowi.persiancalendar.utils.createNRemoteViews
...
class NWidgetConfigurationActivity : BaseWidgetConfigurationActivity() {
    @Composable
    override fun Preview(appWidgetId: Int) {
        val coordinates by coordinates.collectAsState()
        val prayTimes = coordinates?.calculatePrayTimes()
        key(prayTimes) {
            WidgetPreview(100.dp) { context, width, height ->
                val clock = Clock(GregorianCalendar())
                createNRemoteViews(
                    context,
                    width,
