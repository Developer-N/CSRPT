...
fun update(context: Context, updateDate: Boolean) {
    ...
    prayTimes ?: return
    @StringRes val nextOwghatId = prayTimes.getNextOwghatTimeId(nowClock)
...
@StringRes
fun PrayTimes.getNextOwghatTimeId(current: Clock): Int {
...
) = prefs.getString(key, null)?.let(Color::parseColor) ?: DEFAULT_SELECTED_WIDGET_BACKGROUND_COLOR

private fun getWidgetTextColor(
    prefs: SharedPreferences, key: String = PREF_SELECTED_WIDGET_TEXT_COLOR
) = prefs.getString(key, null)?.let(Color::parseColor) ?: DEFAULT_SELECTED_WIDGET_TEXT_COLOR
...
private fun createNRemoteViews(
    ...
    val title = "${context.resources.getString(R.string.today)} :" + " ${
        dayTitleSummary(
            Jdn.today(),
            Jdn.today().toCalendar(mainCalendar)
        )
...
private data class NotificationData(
    ...
    fun post(context: Context): Boolean {
        ...
        val builder = NotificationCompat.Builder(context, notificationId.toString())
            ...
            ).setContentTitle(toPrepend + title).setContentText(
                when {
