...
class SuraViewActivity : AppCompatActivity() {
    ...
    private val model: SuraViewModel by viewModel()
                    ...
                    when (it) {
                        ...
                        QURAN_NOTIFY_VIEW_PLAYER_STOP -> {
                            ...
                            if (binding.cardQuranPalayer.isVisible) {
                                ...
                                ObjectAnimator.ofFloat(
                                    binding.suraRecycler, "translationY", 0f
                                ).apply {
                                    duration = 500
                                    interpolator = AnticipateOvershootInterpolator()
                                    start()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        model.getAyas().observe(this) {
            if (it.size > 0) adapter.setAyas(it)
            if (aya > 0) (binding.suraRecycler.layoutManager as LinearLayoutManager).scrollToPositionWithOffset(
                aya - 1, 0
            )
        }
    ...
    private inner class SuraViewAdapter :
        ...
        override fun getFilter(): Filter {
            return object : Filter() {
                ...
                override fun performFiltering(constraint: CharSequence?): FilterResults {
                    ...
                    for (quranEntity: QuranEntity in quranList!!) if (quranEntity.simple!!.contains(
                            kyFarsiToArabicCharacters(charString)
                        ) || quranEntity.simple_clean!!.contains(
                            kyFarsiToArabicCharacters(
                                charString
                            )
                        ) || quranEntity.en_transilation!!.contains(charString) || quranEntity.en_pickthall!!.contains(
                            charString
                        ) || quranEntity.fa_khorramdel!!.contains(charString) || quranEntity.ku_asan!!.contains(
                            kKurdishToArabicCharacters(charString)
        ...
        inner class SuraItemViewHolder(private val itemBinding: ItemAyaBinding) :
            ...
            fun bind(aya: QuranEntity) {
                ...
                if (aya.aya != 1 || aya.sura == 1 || aya.sura == 9) itemBinding.ayaItemArabic.text =
                    aya.simple
                else {
                ...
                } else itemBinding.ayaItemEnTransilation.visibility = View.GONE
                ...
                } else itemBinding.ayaItemEnglish.visibility = View.GONE
                ...
                } else itemBinding.ayaItemKurdish.visibility = View.GONE
                ...
                } else itemBinding.ayaItemFarsi.visibility = View.GONE
                ...
                if (aya.fav == 1) itemBinding.btnAyaBookmark.setImageResource(R.drawable.ic_bookmark1)
                else itemBinding.btnAyaBookmark.setImageResource(R.drawable.ic_bookmark0)
                ...
                if (filter.isNotEmpty()) {
                    ...
                    if (itemBinding.ayaItemArabic.visibility == View.VISIBLE) itemBinding.ayaItemArabic.text =
                        formatHtmlToSpanned(
                            itemBinding.ayaItemArabic.text.toString().replace(filter, strReplace)
                        )
                    if (itemBinding.ayaItemEnglish.visibility == View.VISIBLE) itemBinding.ayaItemEnglish.text =
                        formatHtmlToSpanned(
                            itemBinding.ayaItemEnglish.text.toString().replace(filter, strReplace)
                        )
                    if (itemBinding.ayaItemEnTransilation.visibility == View.VISIBLE) itemBinding.ayaItemEnTransilation.text =
                        formatHtmlToSpanned(
                            itemBinding.ayaItemEnTransilation.text.toString()
                                .replace(filter, strReplace)
                        )
                    if (itemBinding.ayaItemKurdish.visibility == View.VISIBLE) itemBinding.ayaItemKurdish.text =
                        formatHtmlToSpanned(
                            itemBinding.ayaItemKurdish.text.toString().replace(filter, strReplace)
                        )
                    if (itemBinding.ayaItemFarsi.visibility == View.VISIBLE) itemBinding.ayaItemFarsi.text =
                        formatHtmlToSpanned(
                ...
                itemBinding.btnAyaCopy.setOnClickListener {
                    it.startAnimation(
                        AnimationUtils.loadAnimation(
                            this@SuraViewActivity, com.google.android.material.R.anim.abc_fade_in
                        )
                    )
                    val clipboardService = getSystemService<ClipboardManager>()
                    clipboardService?.setPrimaryClip(
                        ClipData.newPlainText(
                            getString(R.string.quran), description
                        )
                    )
                ...
                itemBinding.btnAyaBookmark.setOnClickListener {
                    ...
                    itemBinding.btnAyaBookmark.setImageResource(
                        if (aya.fav == 1) R.drawable.ic_bookmark1
                        else R.drawable.ic_bookmark0
                    )
                ...
                itemBinding.btnAyaNote.setOnClickListener {
                    ...
                    if (itemBinding.txtAyaNoteLayout.visibility == View.GONE) {
                        ...
                        aya.note = note
                        if (note == "-") it.setImageResource(R.drawable.ic_note_add)
                        else it.setImageResource(R.drawable.ic_note)
                        itemBinding.txtAyaNoteLayout.visibility = View.GONE
                        db.quranDao().update(aya)
                ...
                itemBinding.txtAyaNoteLayout.setEndIconOnClickListener {
                    ...
                    if (aya.note == "-") itemBinding.btnAyaNote.setImageResource(R.drawable.ic_note_add)
                    else itemBinding.btnAyaNote.setImageResource(R.drawable.ic_note)
                    itemBinding.txtAyaNoteLayout.visibility = View.GONE
                    val transient = ChangeBounds().apply {
                        duration = 200
                        interpolator = AccelerateDecelerateInterpolator()
